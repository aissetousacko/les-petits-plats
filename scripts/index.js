...
const displayHandler = () => {
   ...
   if(searchbarValue.length < 3 && ingredientsSelected.length === 0 && appliancesSelected.length === 0 && ustensilsSelected.length === 0) {
   ...
   */
   ingredientsSelected.forEach(tag => {
      recipesFiltered = recipesFiltered.filter(recipe => {
         return recipe.ingredients.some(({ ingredient }) => ingredient.toLowerCase() === tag);
      })
   })
